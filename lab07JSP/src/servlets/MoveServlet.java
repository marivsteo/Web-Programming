...
public class MoveServlet extends HttpServlet {
    ...
    protected synchronized void  doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        int activeUser=(Integer) req.getSession().getAttribute("player");
        char play='x';
        int value;
        ...
        try {
            ...
            if (result.next()) {
                Board = result.getString("state");
                ActivePlayer=result.getInt("player");
            }
        ...
        if(user.getPlayerNo()==activeUser)
        {
            ...
            if(ActivePlayer==0) {
                req.getSession().setAttribute("player", 1);
                String update="UPDATE board SET state='"+stringBuilder.toString()+"', player='"+1+"';";
                try {
                ...
                req.getSession().setAttribute("player", 0);
                String update="UPDATE board SET state='"+stringBuilder.toString()+"', player='"+0+"';";
                try {
                    PreparedStatement stmt = Manager.getConnection().prepareStatement(update);
                    stmt.execute();
        ...
        else
        {
            ...
            req.getSession().setAttribute("player",ActivePlayer);
            if(value==0) {
                resp.sendRedirect("/board.jsp");
            }
            else if(value==1)
