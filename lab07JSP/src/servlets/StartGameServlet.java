...
public class StartGameServlet extends HttpServlet {
    protected synchronized void  doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            req.getSession().setAttribute("nrplayers",PlayersLogged);
            if(PlayersLogged>=2)
            {
    ...
    public void CreateBoard(HttpSession session)
    {
        ...
        try {
            ...
            ResultSet result = stmt.executeQuery();
            if (result.next()) {
                String Board = result.getString("state");
                int ActivePlayer=result.getInt("player");
                session.setAttribute("Board",Board);
                session.setAttribute("player",ActivePlayer);
            }
